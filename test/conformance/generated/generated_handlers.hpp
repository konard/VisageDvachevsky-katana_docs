...
struct api_handler {
    ...
    // POST /items
    // Create an item with strict validation
    virtual katana::result<void> create_item(const CreateItemRequest& body, response& out) = 0;

    // GET /items
    // List items (no body)
    virtual katana::result<void> list_items(std::optional<int64_t> limit, response& out) = 0;

    // GET /items/{id}
    // Get item by path param
    virtual katana::result<void> get_item(int64_t id, response& out) = 0;

    // POST /echo
    // Echo back request body
    virtual katana::result<void> echo_body(const echo_body_request& body, response& out) = 0;
