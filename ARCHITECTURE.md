...
### Наблюдаемость (целевое состояние — Stage 6)

Метрики p50/p95/p99, распределенный трейсинг, структурированные логи с корреляцией trace_id/span_id. На данный момент реализованы только атомарные счётчики в reactor и benchmark harness.
